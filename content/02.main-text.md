...
In general across all genes, the best-performing SGD models tend to be bimodal, sometimes having few nonzero coefficients but often having many/all nonzero coefficients.
By contrast, the `liblinear` models are almost always much sparser with fewer than 2500 nonzero coefficients, out of ~16100 total input features (Figure {@fig:optimizer_coefs}A).
However, the sum of the coefficients for KRAS mutation prediction models tends to be smaller across all levels of regularization for SGD than for `liblinear` (Figure {@fig:optimizer_coefs}B).
These results suggest that the models fit by `liblinear` and SGD navigate the tradeoff between bias and variance in slightly different ways: SGD coefficients tend to have smaller magnitudes as regularization increases, but `liblinear` tends to produce sparser models (more zero coefficients) as regularization increases.

We also compared the components of the loss function across different levels of regularization between optimizers.
The LASSO logistic regression loss function can be broken down into a data-dependent component (the log loss) and a parameter magnitude dependent component (the L1 penalty), which are added to get the total loss that is optimized; see Methods for additional details.
...
This seems to suggest that SGD with a constant learning rate has a form of "implicit regularization" preventing it from achieving extremely low training loss values, which could contribute to its robustness to overfitting.
Other SGD learning rate schedules have loss curves that look more like the `liblinear` results (Supplementary Figure TODO), supporting the hypothesis that the robust performance of SGD with a constant learning rate is related to the higher than expected data loss.
...
**B.** Sum of model coefficient weights across regularization levels (unchanged parameter for SGD and 1 / parameter for `liblinear`). Dots represent the mean sum of the absolute value of coefficient weights, across folds/seeds.
