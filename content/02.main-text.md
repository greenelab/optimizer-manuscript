...
These results suggest that the models fit by `liblinear` and SGD navigate the tradeoff between bias and variance in slightly different ways: `liblinear` tends to produce sparser models (more zero coefficients) as regularization increases, but SGD coefficients tend to have smaller magnitudes as regularization increases.
...
Other SGD learning rate schedules have loss curves that look more like the `liblinear` results, although this does not seem to result in improved classification performance (Supplementary Figure TODO).
