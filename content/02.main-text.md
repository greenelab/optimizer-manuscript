...
We saw that LASSO logistic regression models for mutation status prediction fit using stochastic gradient descent tended to be robust to overfitting at low regularization levels, but they were highly sensitive to learning rate tuning.
Using coordinate descent implemented in `liblinear` to optimize the same models did not require learning rate tuning, but generally resulted in overfitting across different regularization strengths.
Our results suggest that tuning of regularization strength for `liblinear`, and learning rate (and regularization strength to a lesser degree) for SGD, are critical steps in machine learning modeling of transcriptomic data, which must be identified and considered as part of analysis pipelines.
