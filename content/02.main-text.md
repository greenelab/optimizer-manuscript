...
For example, R's `glmnet` package uses a cyclical coordinate descent algorithm to fit logistic regression models [@doi:10.18637/jss.v033.i01], which would presumably behave similarly to `liblinear`, although this is somewhat opaque in the `glmnet` documentation itself.
...
- something about other feature selection/gene expression work and how this relates
