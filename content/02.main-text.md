...
Our results suggest that even for the same model, LASSO logistic regression, optimizer choice presents tradeoffs in model selection. (more summary here)
...
Increased transparency and documentation in popular machine learning packages with respect to optimization, especially for models that are challenging to fit or sensitive to hyperparameter settings, would benefit new and unfamiliar users.
